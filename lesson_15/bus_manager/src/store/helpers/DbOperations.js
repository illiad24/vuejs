...
class DbOperations {
    ...
    loadFilteredData({ filtersOptions }, options) {
        let filters = []
        if (filtersOptions.length > 0)
            filters = filtersOptions.map((option) =>
                where(option.fieldTitle, option.compareOperator, option.valueToCompare)
            )

        const queryOpt = this.getQueryOptions(options, ...filters)
